<template>
  <div class="rightFunction"
       :class="{'toggleRightFunction':toggleRightFunction}">
    <div id="toolTip"
         style="display:none;position:absolute;height:20px;width:127px;background: olive;top:0px;left:0px">
    </div>
    <div class="eyesPanel inline-block">
      <div class="funItem funItemCollapse"
           @click="toggleRightFunction=!toggleRightFunction"><img src="/imgs/icon-collapse.png"></div>
      <div class="funItem"
    ...
    <div class="funsPanel inline-block">
      <!-- 生命工程专题 -->
      <el-form ref="form"
               v-if="selectedItem.type =='smgczt'"
               label-width="20px">
        <el-form-item v-for="(item,i) in lifeProj"
                      :key="i">
          <label class="funSubItem">
            <el-checkbox @change="toggleDzysf($event, item)"
                         v-model="item.select"></el-checkbox>
            <el-image class="funSUbImgs"
      ...
      <div class="mapType"
           v-if="selectedItem.type =='tool'">
        <div @click="toggleMap(subItem.type)"
             :key="i"
             v-for="(subItem,i) in eyesList[0].mapChildren">
          <img :src="subItem.icon"><span :class="{'on': mapSourceName==subItem.type}">{{subItem.name}}</span>
        </div>

        <el-form label-width="10px">
          <el-form-item label="">
            <el-checkbox v-model="mapNameLayer"
      ...
      <div class="toolType"
           v-if="selectedItem.type =='tool'">
        <div @click="toggleTool(item)"
             :key="i"
             v-for="(item,i) in mapTools">
          <div class="toolImgDiv"><img :src="item.icon"></div><span>{{item.title}}</span>
        </div>
      </div>
      <div class="dzysfType"
           v-if="selectedItem.type =='dzysf'">
        <div :key="i"
             v-for="(item, i) in mapLayers">
          <div class="dzysfTitle">{{item.name}}</div>
          <div class="dzysfItem"
               :key="subItem.name"
               v-for="subItem in item.subs">
            <el-checkbox @change="toggleDzysf($event, subItem)"
                         v-model="subItem.select">{{subItem.name}}</el-checkbox>

          </div>
        </div>
      </div>
      <el-form ref="form"
               v-if="selectedItem.type =='zs'"
               label-width="20px">
        <el-form-item v-for="(item,i) in mapCasualtyLossLayer"
                      :key="i">
          <label class="funSubItem">
            <el-checkbox @change="toggleZs($event, item)"
